...
C-NGMM is a natural gas market model patterned on the formulation of EIA's Natural Gas Market
Module in NEMS. It clears a multi-regional market over production, pipeline flow, storage, and
LNG trade against sectoral demand, returning regional prices as duals of the market-balance
constraint.
...
> form is preserved in the docstring of `supply_pbase` in `src/models/natural_gas/data.py`.
...
**Every file below is required.** `src/models/natural_gas/data.py` holds no fallback constants: a missing, unreadable,
...
`src/models/natural_gas/ng_model.py` exposes the methods another model uses to drive it:
...
highspy. No commercial solver is required.

## How this differs from NGMM in NEMS

While the overall market economics are similar, the network topology and the
endogenous capacity decisions are not represented yet. The difference is beyond seasonality and regionality.
...
**This model right now has one layer.** Each census division is simultaneously supply node, hub, and demand
...
There is no separate hub price, no supply-region-to-hub gathering leg distinct from the hub network, and no representation of a demand region served by
...
So a fair description is that this is a faithful reduced-form implementation of NGMM's market-clearing
