...
Common helper functions for C-HSM.

Adapted from ``models/hsm/common.py`` of EIA's National Energy Modeling System (NEMS),
https://github.com/EIAgov/NEMS, release tag ``AEO2025-Public-Release``, licensed under the
Apache License 2.0.
This file has been modified from the NEMS original: ``print_out`` sends its text to the logger
instead of printing it, ``read_dataframe`` takes the file extension after the last dot, warnings
carry ``stacklevel=2``, and the file is formatted to the C-NEMS code style.
...
def print_out(text_in):
    """Routes HSM diagnostic text to the module logger (debug level).

    NEMS prints this with tab indents for the stack depth; here it goes to the logger at
    debug level, so it stays off the console unless the logger is set to DEBUG.

    Parameters
    ----------
    text_in : str, int, float
    """
    logger.debug('%s', text_in)
...
def read_dataframe(filename, sheet_name=0, index_col=None, skiprows=None, to_int=True):
    ...
    # Take the extension after the LAST dot. The NEMS original used split('.')[1], which breaks
    # as soon as a folder name above the file contains a dot.
    extension = filename.rsplit('.', 1)[-1]
    if extension == 'xlsx':
        df = pd.read_excel(filename, sheet_name=sheet_name, index_col=index_col, skiprows=skiprows)
    elif extension == 'hdf':
        # read_hdf does not return a DataFrame; wrap it with pd.DataFrame to satisfy PyCharm
        # (may cause an error)
        df = pd.DataFrame(pd.read_hdf(filename, key=sheet_name))
        if type(index_col) is str:
            df = df.set_index(index_col)
        elif type(index_col) is int:
            df = df.set_index(df.columns[index_col])
    elif extension == 'csv':
        df = pd.read_csv(filename, index_col=index_col, skiprows=skiprows, engine='c')
    else:
        warnings.warn(filename + ' filetype not recognized', UserWarning, stacklevel=2)
...
def assign_ngpls_to_district(df, ngpl_shares, steo_years):
    ...
    pd.DataFrame
    """
    df = ngpl_shares[[nam.ngpl_region_number, nam.ngpl_share_ratio]].merge(
        df, how='left', left_on=nam.ngpl_region_number, right_index=True
    )
...
def df_interpolate_2(left, right, xcol, ycol, xdir='nearest', ydir='nearest', xmerge='merge_asof'):
    ...
    if xmerge == 'merge_asof':
        df = pd.merge_asof(df_in, right[[xcol, xcol + '_round']], on=xcol, direction=xdir)
    elif xmerge == 'merge':
        # basically no rounding necessary, just copy the column with _round
        df = df_in
        df[xcol + '_round'] = df[xcol].copy()
    else:
        warnings.warn('Invalid merge type in offshore interpolation', UserWarning, stacklevel=2)
        exit()  # noqa: PLR1722 - NEMS code, offshore only (not run by C-HSM)
...
def df_to_array(df):
    ...
    except:  # noqa: E722 - 2-dimensional DF, as in NEMS
        array = np.array(df)
