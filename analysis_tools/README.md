...
The viewer is setup to only view electricity related results right now, such as generation, capacity, and trade. After each electricity run, a separate electricity folder will output results within the timestamped run folder. The viewer python code can go through each of the run folders and append the variables for the charts. The [tech_colors.csv](/analysis_tools/tech_colors.csvlors.csv) controls the mapping of technology types to colors. 
